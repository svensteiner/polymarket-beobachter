...
| 2026-07-29 | B2-Fortsetzung | ERLEDIGT (Blocker präzise umbenannt, weiterhin nicht testbar) — `analytics/arb_partition_coverage.py`. **Durchbruch beim Partitions-Problem:** Der Collector persistiert `negRiskMarketID` bereits in jedem Roh-Snapshot; alle Buckets eines negRisk-Events teilen sie und sind per Konstruktion disjunkt+vollständig. Damit ist die Partitions-Zugehörigkeit **ex-ante & autoritativ** bekannt — die alte `(city,date,metric)`-Heuristik samt Look-ahead-Risiko ist überflüssig. **383 negRisk-Partitionen** rekonstruiert, **78** im beobachteten+aufgelösten Universum. **ABER: der wahre Blocker ist Preis-Coverage, nicht Partitions-Unkenntnis:** wir bepreisen im Beobachtungs-Log median nur **18,2%** (max **45,5%**) der Buckets einer typisch 11er-Partition; **0/78 vollständig bepreist**. S = ΣYES-Preis ist also nie bildbar. Damit ist auch der alte „nächste Schritt" (Markt-IDs persistieren) widerlegt — das allein reicht NICHT. Modul liefert automatisch validen Arb-Test, sobald Coverage→100%. In `edge_routine` verdrahtet (Step + Digest-Zeile). | (lokal) |
...
- ~~**B2-Fortsetzung** (Partition ex-ante prüfbar machen)~~ **ERLEDIGT 2026-07-29** — Partitions-Zugehörigkeit ist via `negRiskMarketID` gelöst (`analytics/arb_partition_coverage.py`), aber der wahre Blocker ist **Preis-Coverage** (median 18,2%, 0/78 vollständig), nicht Partitions-Unkenntnis. Markt-IDs persistieren allein reicht NICHT.
- **B2-PRICE-COVERAGE (neuer, präzisierter Nachfolge-Auftrag, aussichtsreichster modell-/regime-unabhängiger Pfad):** Damit die Cross-Market-Arbitrage überhaupt testbar wird, muss der Observer für **jeden** Bucket einer negRisk-Partition einen Preis-Snapshot zum gemeinsamen Lead schreiben (nicht nur für handelbar gefundene Buckets). Das ist eine **Forward-Datenerfassung**, berührt den 15-Min-Zyklus (Guardrail 8) → sorgfältig, klein, fail-open umsetzen; Menschen-Freigabe für Zyklus-Eingriff einholen. Sobald Coverage→100%, liefert `arb_partition_coverage.py` ohne weitere Änderung den validen, look-ahead-freien Arb-Test. **Retrospektiv nicht schließbar:** Der Sanitizer strippt alle Preisfelder aus den Roh-Snapshots (0/500 haben `outcomePrices`/`bestBid`/`bestAsk`), die einzige historische Preisquelle ist das Beobachtungs-Log mit ~18% Coverage. Es bleibt also nur der Forward-Weg. Umsetzung ist billig: pro entdeckter negRisk-Partition die `outcomePrices` **aller** Buckets im gleichen Zug snapshotten, den der Collector ohnehin fetcht — ein schlanker, read-only Zusatz-Log neben dem Beobachtungs-Log, KEINE Sanitizer-Lockerung im normalen Pfad.
