...
# =============================================================================
# FORBIDDEN FIELDS - These MUST NOT appear in MarketInput
# =============================================================================
# GOVERNANCE: These fields are explicitly forbidden because they represent
# real-time market data that would violate Layer 1's "no prices" rule.
#
# NOTE: market_implied_probability is ALLOWED but documented as a SANITY CHECK
# field only - it does not influence the TRADE/NO_TRADE decision logic itself.
# =============================================================================
...
class MarketInput:
    ...
    GOVERNANCE NOTE:
    This class is IMMUTABLE after creation to prevent field injection attacks.
    Forbidden fields (price, volume, liquidity, pnl, etc.) will be REJECTED.
    ...
    - market_implied_probability: Market probability for SANITY CHECK ONLY
      (0.0 to 1.0) - used to detect divergence, NOT for decision logic
    - analysis_date: Date when this analysis is performed
    - notes: Optional additional context

    PRICE DATA POLICY:
    The market_implied_probability field IS derived from market prices.
    However, it is used ONLY for the sanity check threshold comparison
    (15 percentage point divergence check). It does NOT influence whether
    the market is structurally tradeable - that decision is based purely
    on resolution quality, timeline feasibility, and process stage.
    """
    market_title: str
    resolution_text: str
    target_date: date
    ...
    market_implied_probability: float  # 0.0 to 1.0 - SANITY CHECK ONLY
    analysis_date: date
    notes: Optional[str] = None

    # Internal flag to track if initialization is complete
    ...
    def __post_init__(self):
        ...
        # Mark initialization as complete - enables immutability
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        GOVERNANCE: Makes the dataclass effectively immutable to prevent
        injection of forbidden fields via __dict__ manipulation.
        """
        # Check for forbidden fields
        if name.lower() in FORBIDDEN_INPUT_FIELDS:
            raise ValueError(
                f"GOVERNANCE VIOLATION: Field '{name}' is FORBIDDEN in MarketInput. "
                f"Forbidden fields: {sorted(FORBIDDEN_INPUT_FIELDS)}"
            )
        ...
        if not self.__dict__.get('_initialized', False):
            ...
            raise AttributeError(
                f"MarketInput is immutable. Cannot modify field '{name}' after creation."
            )

    def __delattr__(self, name: str) -> None:
        """Prevent deletion of attributes."""
        raise AttributeError(
            f"MarketInput is immutable. Cannot delete field '{name}'."
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Remove internal field from serialization
